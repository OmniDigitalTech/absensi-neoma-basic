            ...
            <div class="p-4">
                <form method="post" action="{{ url('/pegawai/shift/proses-tambah-shift') }}">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control selectpicker @error('shift_id') is-invalid @enderror" id="shift_id"
                            name="shift_id" data-live-search="true">
                            <option value="">Pilih Shift</option>
                            @foreach ($shift as $s)
                            @if(old('shift_id') == $s->id)
                            <option value="{{ $s->id }}" selected>
                                {{ $s->nama_shift . " (" . $s->jam_masuk . " - " . $s->jam_keluar . ") " }}</option>
                            @else
                            <option value="{{ $s->id }}">
                                {{ $s->nama_shift . " (" . $s->jam_masuk . " - " . $s->jam_keluar . ") " }}</option>
                            @endif
                    ...
                    <div class="form-check mb-4">
                        <input name="lock_location" class="form-check-input lock_location" type="checkbox"
                            value="{{ old('lock_location', 1) }}" id="lock_location">
                    ...
                    <table id="mytable" class="table table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $sk->tanggal}}</td>
                                <td>{{ $sk->Shift->nama_shift ?? 'Perlu Konfirmasi Admin'}}</td>
                                <td>{{ $sk->Shift->jam_masuk ?? 'Perlu Konfirmasi Admin'}}</td>
                                <td>{{ $sk->Shift->jam_keluar ?? 'Perlu Konfirmasi Admin'}}</td>
                                <td>
                                    <ul class="action">
                                        <li class="edit">
                                            <a href="{{ url('/pegawai/edit-shift/'.$sk->id) }}"><i
                                                    class="fa fa-solid fa-edit"></i></a>
                                        </li>
                                        <li class="delete">
                                            <form action="{{ url('/pegawai/delete-shift/'.$sk->id) }}" method="post"
                                                class="d-inline">
                                                @method('delete')
                                                @csrf
                                                <input type="hidden" name="user_id" value="{{ $karyawan->id }}">
                                                <button type="button" title="Delete Shift" class="border-0 delete-btn"
                                                    data-item-name="Shift" style="background-color: transparent;"><i
...
$(function() {
    // document.ready(function() {
        let lock_location = $('#lock_location').val();
